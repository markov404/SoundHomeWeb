...
from reviews.components.interfaces.IScruber import IScruber
...
class PitchForkScruber(IScruber):
    def __init__(self, driver: SeleniumAdapter, home_url: str = None) -> None:
    ...
    def update_data(self) -> None:
        ...
        self.__driver.go_to_page(self.__home_path, 20)
        review_cards = self.__driver.get_all_elements(
            (By.CLASS_NAME, "review"))[:12]

        self.__update_all_links(review_cards)
        self.__get_all_information_about_each_review()

    def get_actual_data(self):
        return self.__buffer['data']
        ...
        for link in self.__buffer['list_of_urls']:
            ...
            try:
                items = self.__driver.get_all_elements(
                    (By.CSS_SELECTOR,
                     ".GridItem-btGJuM.iUhqhB.grid--item")
                )
                title_of_review = self.__driver.get_all_elements(
                    (By.CSS_SELECTOR,
                     ".BaseWrap-sc-SJwXJ.BaseText-fEohGt.SplitScreenContentHeaderDekDown-flUiFp.deUlYF.dZBepS.gRuqoV"))
                actual_review_text = self.__driver.get_all_elements(
                    (By.CSS_SELECTOR,
                     ".BodyWrapper-cuyyBm.iBJDMQ.body.body__container.article__body"))

                actual_review_text = [
                    el.get_attribute('innerHTML') for el in actual_review_text]
    ...
    def __get_review_text(self, review_text: list[str]) -> dict:
        ...
        for html in review_text:
            ...
            output = output.split(
                "All products featured on Pitchfork are independently selected by our editors.")[0]
